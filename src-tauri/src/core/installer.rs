...
impl Installer {
    pub fn install_from_local(
        ...
        let skill_md = fs_utils::find_skill_marker(source_path).ok_or_else(|| {
        ...
        if name.is_empty() && frontmatter.get("name").is_none() {
            return Err(AppError::Validation(
                "Skill name is required when SKILL.md has no 'name' field".into(),
            ));
        }
    ...
    pub fn install_from_local_zip(
        ...
        if !skill_dir.exists() {
            return Err(AppError::InstallFailed(format!(
                "Extracted skill directory not found: {}",
                skill_root.display()
            )));
        }
